...
class EnvProposed_erf(gym.Env):
    ...
    # BLER estimation
            ...
            for _ in range(int(block_num)):
                ...
                while is_trans_success == 0:
                    ...
                    if is_trans_success == 1 or re_trans_num_packet >= self.max_re_trans_num:
                        ...
                        re_trans_num_packet += 1
                        per = 1 - (1 - bler) ** (self.sub_block_length / (1 - coding_rate))
                self.re_trans_num += re_trans_num_packet
            re_trans_delay = self.re_trans_num * ((1 / coding_rate - 1) * self.sub_block_length / trans_rate)
        ...
        if self.step_num >= self.slot_num - 1:
            ...
            print("Retransmissions per packet:", episode_re_trans_num)
    ...
    def input_est_err(self, est_err_para):
        ...
        mcs_pattern = r'mcs_(\d+)_snr_([\d.]+)_([\d.]+)_([\d.]+)_esterr_([\d.]+)'
        self.mcs_models = {}

        for filename in os.listdir(self.mcs_folder_path):
            if filename.endswith('.mat'):
                match = re.match(mcs_pattern, filename)
                if match:
                    mcs_index = int(match.group(1))
                    snr_min = float(match.group(2))
                    snr_int = float(match.group(3))
                    snr_max = float(match.group(4))
                    est_err = float(match.group(5))
                    if est_err != self.est_err_para:
                        continue
                    print(f"Loading MCS-{mcs_index} SNR-BLER data:", filename)
                    mcs_file_path = os.path.join(self.mcs_folder_path, filename)
                    mcs_data = loadmat(mcs_file_path)
                    bler = mcs_data['bler'].ravel()
                    snr_list = np.arange(snr_min, snr_max + snr_int, snr_int)[:len(bler)]
                    coeffs = np.polyfit(snr_list, bler, deg=5)
                    self.mcs_models[mcs_index] = np.poly1d(coeffs)
