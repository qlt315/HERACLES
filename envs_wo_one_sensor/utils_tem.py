...
def per_list_gen(tm_ber, hm_ber_1, hm_ber_2, data_size, tm_rate, hm_rate):
    ...
    per_list = [dual_camera_per.item(), radar_lidar_per.item(),
                camera_lidar_per.item(),
                dual_camera_per.item(), radar_lidar_per.item(),
                camera_lidar_per.item(),
                dual_camera_per.item(), radar_lidar_per.item(),
                camera_lidar_per.item(),
...
def acc_normalize(acc_exp, current_context):
    ...
    indices_to_remove = [0, 1, 2, 6, 7, 8, 12, 13, 14]
    acc_sunny_list = platform_data["sunny"][:21]
    acc_sunny_list = np.delete(acc_sunny_list, indices_to_remove)
    acc_rain_list = platform_data["rain"][:21]
    acc_rain_list = np.delete(acc_rain_list, indices_to_remove)
    acc_snow_list = platform_data["snow"][:21]
    acc_snow_list = np.delete(acc_snow_list, indices_to_remove)
    acc_motorway_list = platform_data["motorway"][:21]
    acc_motorway_list = np.delete(acc_motorway_list, indices_to_remove)
    acc_fog_list = platform_data["fog"][:21]
    acc_fog_list = np.delete(acc_fog_list, indices_to_remove)
    acc_night_list = platform_data["night"][:21]
    acc_night_list = np.delete(acc_night_list, indices_to_remove)
...
def action_gen():
    ...
    action_sunny_list = [ActionDic(i) for i in range(1, 25)]  # 12 + 12 = 24 actions for each context
    ...
    fusion_name_list = [[1, 2], [2, 1], [3, 4], [4, 3], [1, 4], [4, 1],
                        [1, 2], [2, 1], [3, 4], [4, 3], [1, 4], [4, 1],
                        [1, 2], [2, 1], [3, 4], [4, 3], [1, 4], [4, 1],
                        [3, 4], [4, 3], [1, 2], [2, 1], [1, 4], [4, 1]]

    mod_type_list = ["HM", "HM", "HM", "HM", "HM", "HM",
                     "HM", "HM", "HM", "HM", "HM", "HM",
                     "HM", "HM", "HM", "HM", "HM", "HM",
                     "HM", "HM", "HM", "HM", "HM", "HM"]
    ...
    def action_list_gen(acc_list):
        ...
        a = acc_list
        action_acc_list = [a[0], a[0], a[1], a[1], a[2], a[2],
                           a[3], a[3], a[4], a[4], a[5], a[5],
                           a[6], a[6], a[7], a[7], a[8], a[8],
                           a[9], a[9], a[10], a[10], a[11], a[11]
                           ]
    ...
    stem_delay_list = [stem_dual_camera_delay, stem_dual_camera_delay, stem_radar_lidar_delay, stem_radar_lidar_delay,
                       stem_camera_lidar_delay, stem_camera_lidar_delay, stem_dual_camera_delay, stem_dual_camera_delay,
                       stem_radar_lidar_delay, stem_radar_lidar_delay, stem_camera_lidar_delay, stem_camera_lidar_delay,
                       stem_dual_camera_delay, stem_dual_camera_delay, stem_radar_lidar_delay, stem_radar_lidar_delay,
                       stem_camera_lidar_delay, stem_camera_lidar_delay,
    ...
    stem_energy_list = [stem_dual_camera_energy,
    ...
    branch_delay_list = np.delete(branch_delay_list, indices_to_remove)
    branch_energy_list = platform_data["branch_energy"][:21]
    branch_energy_list = np.delete(branch_energy_list, indices_to_remove)
