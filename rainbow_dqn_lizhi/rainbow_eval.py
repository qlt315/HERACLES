...
class Runner:
    def __init__(self, args, env_name, number, seed):
        ...
        self.env_evaluate = EnvProposed()  # When evaluating the policy, we need to rebuild an environment
        # self.env.seed(seed)
        ...
        self.total_steps = 0  # Record the total steps during the evaling
...
if __name__ == '__main__':
    ...
    for k in range(len(seed_list)):
        ...
        if runner.env.name == "proposed":
            runner.agent.net = torch.load('rainbow_dqn_lizhi/models/rainbow_dqn_proposed.pth')
            runner.agent.target_net = torch.load('rainbow_dqn_lizhi/models/rainbow_dqn_target_proposed.pth')
        elif runner.env.name == "sse":
            runner.agent.net = torch.load('rainbow_dqn_lizhi/models/rainbow_dqn_sse.pth')
            runner.agent.target_ne = torch.load('rainbow_dqn_lizhi/models/rainbow_dqn_target_sse.pth')
        elif runner.env.name == "tem":
            runner.agent.net = torch.load('rainbow_dqn_lizhi/models/rainbow_dqn_tem.pth')
            runner.agent.target_ne = torch.load('rainbow_dqn_lizhi/models/rainbow_dqn_target_tem.pth')

        runner.run()

        # save the data
        if runner.env.name == "proposed":
            mat_name = "rainbow_dqn_lizhi/eval_data/eval_proposed_data.mat"
        elif runner.env.name == "sse":
            mat_name = "rainbow_dqn_lizhi/eval_data/eval_sse_data.mat"
        elif runner.env.name == "tem":
            mat_name = "rainbow_dqn_lizhi/eval_data/eval_tem_data.mat"
