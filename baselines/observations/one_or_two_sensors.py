...
class EnvTEM(gym.Env):
    def __init__(self):
        ...
        #     self.data_size[0, i] = (np.random.rand(1) * 5000).item()
        self.data_size = np.zeros([1, self.sensor_num])
        ...
        self.context_interval = 1  # Interval for context to change
    ...
    def step(self, action):
        ...
        if len(action_info.fusion_name) == 1:
            coded_data_size = self.data_size[0, data_size_idx] / self.tm_coding_rate
        else:
            for j in range(len(action_info.fusion_name)):
                coded_data_size = coded_data_size + self.data_size[0, action_info.fusion_name[j]-1] / self.tm_coding_rate
        ...
        trans_delay = coded_data_size / tm_trans_rate + re_trans_delay
        ...
        self.total_delay_list[0, self.step_num] = total_delay.item()

        # Calculate energy consumption
        com_energy = action_info.com_energy
        total_energy = trans_energy + com_energy
        self.total_energy_list[0, self.step_num] = total_energy.item()
        self.remain_energy = self.remain_energy - total_energy
        print("com energy:", com_energy,"trans energy:",trans_energy,"re trans energy:",re_trans_energy)
        #         # Reward calculation
        ...
        self.acc_exp_list[0, self.step_num] = acc_exp[0]
        # acc_exp = util.acc_normalize(acc_exp, self.curr_context)
        if acc_exp < min_acc:
        ...
        self.reward_list[0, self.step_num] = reward.item()
...
if __name__ == "__main__":
    ...
    seed_list = [37]
    data = loadmat('baselines/observations/observation.mat')
    obs_energy = data["obs_energy"]
    ...
    for a in range(len(action_list)):
        for s in range(len(snr_db_list)):
            ...
            state = env.reset()
            while not env_done:
                    action = action_list[a] # 0 - 20, totally 21 actions
                    next_state, reward, done = env.step(action)
                    env_done = done
                    # env.step_num = env.step_num + 1
            obs_energy[a, s] = env.total_energy_list[0,0]
            obs_delay[a, s] = env.total_delay_list[0,0]
            obs_re_trans[a, s] = env.re_trans_num
